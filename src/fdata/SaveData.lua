		...
		local function Scan(Object, Data)
			for i, v in pairs(Object:GetChildren()) do
				...
				elseif v:IsA("RayValue") then
					Data[v.Name] = {
						["ClassName"] = v.ClassName;
						["x"] = sigFigs(v.Value.Origin.X);
						["y"] = sigFigs(v.Value.Origin.Y);
						["z"] = sigFigs(v.Value.Origin.Z);
						["dx"] = sigFigs(v.Value.Direction.X);
						["dy"] = sigFigs(v.Value.Direction.Y);
						["dz"] = sigFigs(v.Value.Direction.Z);
						["Attributes"] = saveAttributes(v);
					}
